...
export class AnalyticsService {
  ...
  async aggPerUser() {
    ...
    SELECT
      ...
      COUNT(*) FILTER (WHERE b.bet_outcome='win') AS total_wins
    FROM bets b
    JOIN "user" u ON u.user_id = b.user_id
  ...
  async sportAnalytics() {
    ...
    SELECT
      ...
      SUM(b.bet_amount) AS total_bet_amount,
      ROUND(
        COUNT(*) FILTER (WHERE b.bet_outcome='win')::decimal 
        / COUNT(*) * 100, 2
      ) AS win_rate
    FROM bets b
    JOIN events e ON e.event_id = b.event_id
  ...
  async dailyAnalytics() {
    ...
    SELECT
      ...
      COUNT(*) AS total_bets,
      SUM(b.bet_amount) AS total_bet_amount,
      SUM( b.bet_amount) FILTER (WHERE b.bet_outcome='win')
        AS total_winning_amount
    FROM bets b
    WHERE b.bet_date >= CURRENT_DATE - INTERVAL '30 days'
    GROUP BY day
    ORDER BY day
  `;
  }
  ...
  async cricketBetters() {
    ...
    SELECT
      ...
      COUNT(*) FILTER (WHERE b.bet_outcome='win') AS total_cricket_wins
    FROM bets b
    JOIN events e ON e.event_id = b.event_id
    JOIN "user" u ON u.user_id = b.user_id
    GROUP BY  u.user_id, u.username
    HAVING 
    COUNT(DISTINCT e.sport) = 1
    AND MAX(e.sport)= 'cricket'
  `;
  }
  ...
  WITH active_users AS (
    ...
    FROM bets b
    JOIN active_users au ON au.user_id = b.user_id
    JOIN "user" u ON u.user_id = b.user_id
  `;
  }
  ...
  async highRiskUsers() {
    ...
    HAVING
      COUNT(*) > 50 AND
      COUNT(*) FILTER (WHERE b.bet_outcome='win')::decimal / COUNT(*) < 0.2 AND
